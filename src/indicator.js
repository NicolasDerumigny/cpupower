...
export class CPUFreqIndicator extends baseindicator.CPUFreqBaseIndicator {
    ...
    enable() {
        this.power = Main.panel.statusArea.quickSettings._system._systemItem._powerToggle;
        ...
        this.timeout = GLib.timeout_add_seconds(GLib.PRIORITY_DEFAULT, 1, () => this.updateFreq());
        this.timeoutMinMax = GLib.timeout_add_seconds(GLib.PRIORITY_DEFAULT, 1, () => this.updateFreqMinMax(false));
    }
        ...
        utils.Cpufreqctl.backends.current(backend, (result) => {
            if (!result.ok) {
                ...
                default:
                    this.showError(
                        `${_("Oh no! Something went wrong.")}\n` +
                            `${_("An internal error occurred:")} ${utils.Cpufreqctl.exitCodeToString(result.exitCode)}`,
                        true,
                    );
                }
            } else {
    ...
    updateMax(done) {
        let backend = this.settings.get_string("cpufreqctl-backend");
        utils.Cpufreqctl.max.set(backend, Math.floor(this.maxVal).toString(), (_result) => {
            this.updateFile();
    ...
    updateMin(done) {
        let backend = this.settings.get_string("cpufreqctl-backend");
        utils.Cpufreqctl.min.set(backend, Math.floor(this.minVal).toString(), (_result) => {
            this.updateFile();
    ...
    updateTurbo(done) {
        let backend = this.settings.get_string("cpufreqctl-backend");
        utils.Cpufreqctl.turbo.set(backend, this.isTurboBoostActive ? "on" : "off", (_result) => {
            this.updateFile();
